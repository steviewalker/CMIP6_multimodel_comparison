#' @title Calculate time series of globally integrated POC flux change at 1000m (1850-2100) for UKESM
...
UKESM_1000_time_series_his <- function(file, wd, model.name, area.name, start.date, end.date, year.seq, section) {
  ...
  write.csv(df,paste0("~/senior_thesis/plotting_dataframes/time_series/UKESM_1000_time_series_",section,".csv"))
  
}


UKESM_1000_time_series_fut <- function(file, wd, model.name, area.name, start.date, end.date, year.seq, section) {
  
  #read in POC flux data, MLD arrays calculated in calc_time_series_MLDmax.R
  setwd(wd)
  nc_data <- nc_open(file)
  
  MLD.fut <- readRDS(paste0("~/senior_thesis/plotting_dataframes/time_series/UKESM_MLD_fut_time_series.Rds"))
