...
calc_MLD_max <- function(wd, nc.file, seq.start.st, seq.start.lt, model.name) {
  ...
  Y1 <- array(Y1, dim=c(dim(list_max[[1]]), length(list_max)))
  
  #20 year mean for the beginning of the 21st century
  mean_MLD_max_st <- apply(Y1, c(1, 2), mean, na.rm = FALSE)
  ...
  Y2 <- array(Y2, dim=c(dim(list_max[[1]]), length(list_max)))
  
  #20 year mean for the beginning of the 21st century
  mean_MLD_max_lt <- apply(Y2, c(1, 2), mean, na.rm = FALSE)
  
  MLD_change = mean_MLD_max_lt - mean_MLD_max_st
  
  #saving rds objects for plotting
  ...
  saveRDS(Y1, file = paste(model.name,"_array_MLD_max_st.Rds",sep=""), ascii = TRUE)
  saveRDS(Y2, file = paste(model.name,"_array_MLD_max_lt.Rds",sep=""), ascii = TRUE)
