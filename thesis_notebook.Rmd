...
write_csv(df,"~/senior_thesis/download_indexes/historical_UKESM_expc.csv")
...
model.scenarios <- list.files("~/senior_thesis/CESM_data/", pattern = "*_gn_*.*nc$") %>% 
  ...
  grep("mlotst_Omon_CESM2_historical_r10i1p1f1",., value = TRUE) %>% 
  unique()
...
model.scenarios <- list.files("~/senior_thesis/GFDL_data/", pattern = "*_gr_*.*nc$") %>% 
  ...
  grep("mlotst_Omon_GFDL-ESM4_historical_r1i1p1f1",., value = TRUE) %>% 
  unique()
...
model.scenarios <- list.files("~/senior_thesis/UKESM_data/", pattern = "*_gn_*.*nc$") %>% 
  ...
  unlist %>% 
  #switch between variables and model reps here. You can do this manually here, then rerun. ( Note on 2/8/21)
  grep("mlotst_Omon_UKESM1-0-LL_historical_r1i1p1f2",., value = TRUE) %>% 
  unique()
...
model.scenarios <- list.files("~/senior_thesis/cmip6_data/", pattern = "*_gn_*.*nc$") %>% 
  ...
  unlist %>% 
  #switch between variables and model reps here. You can do this manually here, then rerun. ( Note on 2/8/21)
  grep("pp_Omon_CMCC-ESM2_ssp585_r1i1p1f1",., value = TRUE) %>% 
  unique()
...
#historical time
get_time(data.path = "~/senior_thesis/combined_GFDL_files/",
         file.name = 'expc_Oyr_GFDL-ESM4_historical_r1i1p1f1_gn_NA-NA.nc')
...
9. Calculate maximum annual MLD in the short-term (2014-2034) and long-term (2077-2097) units: m
```{r}
setwd("~/senior_thesis/")
...
18. Calculate global POC flux at 100m
```{r}
setwd("~/senior_thesis/")
...
ggsave(grid.combine, filename = "faceted_expc_200.png", device = "png", path = "~/senior_thesis/figures/", 
       dpi = 500, width = 16, height = 12, units = "cm")
...
ggsave(grid.combine, filename = "faceted_expc_500.png", device = "png", path = "~/senior_thesis/figures/", 
       dpi = 500, width = 16, height = 12, units = "cm")
...
ggsave(grid.combine, filename = "faceted_expc_1000.png", device = "png", path = "~/senior_thesis/figures/", 
       dpi = 500, width = 16, height = 12, units = "cm")
...
test <- get_time(data.path = "~/senior_thesis/combined_EC-Earth_files/",
...
#PLOTTING NORMALIZED 100M AND MLDMAX IN SAME PLOT -------
...
n_1000_all <- read_csv("~/senior_thesis/plotting_dataframes/time_series/normalized_time_series_1000.csv")
...
ggsave(grid.combine, filename = "faceted_DH_comparison.png", device = "png", path = "~/senior_thesis/figures/", 
       dpi = 500, width = 14, height = 12, units = "cm")
