...
Plot 6 panel figure showing POC flux change at MLDmax for all models
```{r}
setwd("~/senior_thesis/figures/")
...
grid.combine <- grid.arrange(rasterGrob(A),rasterGrob(B),rasterGrob(C),
                             rasterGrob(D),rasterGrob(E),ncol=2, nrow=3)

ggsave(grid.combine, filename = "faceted_expc.png", device = "png", path = "~/senior_thesis/figures/", 
       dpi = 500, width = 16, height = 12, units = "cm")
```

Calculate and plot change in POC flux difference (MLDmax - 100m) - 3 panel fig for each model and 6 panel fig of all models 
```{r}
setwd('~/senior_thesis')
...
ggsave(grid.combine, filename = "faceted_expc-epc100.png", device = "png", path = "~/senior_thesis/figures/", 
       dpi = 500, width = 16, height = 12, units = "cm")
...
Plot CESM depth profiles in various locations
```{r}
