...
idx <- init_cmip6_index(
  ...
  # specify GCM names
  source = c("EC-Earth3-CC","MPI-ESM1-2-HR"),
...
write_csv(df,"~/senior_thesis/download_indexes/historical.csv")
...
model.scenarios <- list.files("~/senior_thesis/CESM_data/", pattern = "*_gn_*.*nc$") %>% 
  ...
  grep("expc_Oyr_CESM2_historical_r10i1p1f1",., value = TRUE) %>% 
  unique()
...
model.scenarios <- list.files("~/senior_thesis/GFDL_data/", pattern = "*_gr_*.*nc$") %>% 
  ...
  grep("expc_Oyr_GFDL-ESM4_historical_r1i1p1f1",., value = TRUE) %>% 
  unique()
...
model.scenarios <- list.files("~/senior_thesis/UKESM_data/", pattern = "*_gn_*.*nc$") %>% 
  ...
  unlist %>% 
  #switch between variables and model reps here. You can do this manually here, then rerun. ( Note on 2/8/21)
  grep("expc_Omon_UKESM1-0-LL_historical_r1i1p1f2",., value = TRUE) %>% 
  unique()
...
model.scenarios <- list.files("~/senior_thesis/EC_Earth_data/", pattern = "*_gn_*.*nc$") %>% 
  ...
  unlist %>% 
  #switch between variables and model reps here. You can do this manually here, then rerun. ( Note on 2/8/21)
  grep("epc100_Omon_EC-Earth3-CC_historical_r1i1p1f1",., value = TRUE) %>% 
  unique()
...
model.scenarios <- list.files("~/senior_thesis/MPI_data/", pattern = "*_gn_*.*nc$") %>% 
  ...
  unlist %>% 
  #switch between variables and model reps here. You can do this manually here, then rerun. ( Note on 2/8/21)
  grep("epc100_Omon_MPI-ESM1-2-HR_historical_r1i1p1f1",., value = TRUE) %>% 
  unique()
...
source('libraries.R')
source('get_time.R')
source('calc_global_time_series.R')
...
#GFDL historical
get_time(data.path = "~/senior_thesis/combined_GFDL_files/",
         file.name = 'epc100_Omon_GFDL-ESM4_historical_r1i1p1f1_gn_185001-201412.nc')
...
time_series_100(wd = "~/senior_thesis/combined_GFDL_files/",
                model.name = "GFDL",
                his.name = 'epc100_Omon_GFDL-ESM4_historical_r1i1p1f1_gn_185001-201412.nc',
